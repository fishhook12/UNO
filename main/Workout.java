...
public class Workout {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //cariable used to determine if the decks will be shuffled together or not
        boolean shuffleDecks = true;
        ...
        while(running){
            ...
            String shuffleInput = "0";
            Shuffle:
            while(!deckInput.equals("1") || !deckInput.equals("2")){
                System.out.println("Do you want to shuffle the decks together or play them seperately");
                System.out.println("1.Together");
                System.out.println("2.Seperate");
                shuffleInput = in.nextLine();
                switch (shuffleInput) {
                    case "1":
                        shuffleDecks = true;
                        break Shuffle;
            ...
            {
                newDeck.buildDeck(1,actions);
                decks--;
            }
            System.out.println("Gathering the Deck(s)...");
            System.out.println("\nNow Shuffling\n");
            System.out.println("------------------------------------");
            ...
            Routine:
            while(!playDeck.isEmpty() || !(decks == 0)){
                ...
                while(!inputHand.equals("1") || !inputHand.equals("2")){
                    ...
                    if(inputHand.equals("1")){
                        ...
                        newHand.sortByColor();
                        Output.OutputHand(newHand, (playDeck.isEmpty() && (decks == 0)), playDeck);
                    }
                    //check to see if the decks were not shuffled together and how many decks are left to play
                    /**
                    ...
                    if(playDeck.isEmpty() && !(decks == 0))
                    {
                        System.out.println("Getting next deck...");
                        newDeck.buildDeck(1, actions);
                        decks--;
                        newDeck.shuffleDeck();
                        playDeck.buildPlayDeck(newDeck);
                    }
                    else if(playDeck.isEmpty() && (decks == 0))
                    {
                        break Routine;
                    }
                }
